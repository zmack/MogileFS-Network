package MogileFS::ReplicationPolicy::MultipleHostsWithIgnore;
use strict;
...
sub ignore_devices  { $_[0]{ignore_devices} }
...
sub replicate_to {
    ...
    my $ignore_devices = $self->{ignore_devices};
...
sub unique_hosts {
    my $devs = shift;
    my %ignore_devices = %{$_[0]};
    my %host;  # hostid -> 1
    foreach my $devid (keys %$devs) {
        if ( $ignore_devices{$devid} ) {
          next;
        }
        my $dev = $devs->{$devid};
